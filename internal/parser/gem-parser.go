...
// Initialize package
func initGemPackages(p *model.Package, metadata GemMetadata) *model.Package {
...
func parseGemPackageURL(_package *model.Package) {
	_package.PURL = model.PURL(scheme + ":" + gem + "/" + _package.Name + "@" + _package.Version)
}
