...
func TestAppendGoBinPackage(t *testing.T) {
	...
	for i, test := range tests {
		...
		if Packages[i].Name != test.expected.Name ||
			Packages[i].Type != test.expected.Type ||
			Packages[i].Path != test.expected.Path ||
			Packages[i].Version != test.expected.Version ||
			Packages[i].Description != test.expected.Description ||
			len(Packages[i].Licenses) != len(test.expected.Licenses) ||
			len(Packages[i].Locations) != len(test.expected.Locations) ||
			len(Packages[i].CPEs) != len(test.expected.CPEs) ||
			string(Packages[i].PURL) != string(test.expected.PURL) {
			t.Errorf("Test Failed:\n Expected output of %v \n, Received: %v \n", test.expected, Packages[i])
		}
	}
}
