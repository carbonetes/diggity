...
func TestInitDebianPackage(t *testing.T) {
	...
	for _, test := range tests {
		...
		outputMetadata := output.Metadata.(DebianMetadata)
		expectedMetadata := test.expected.Metadata.(DebianMetadata)
...
func TestParseDebianFiles(t *testing.T) {
	_debMetadata1 := DebianMetadata{"Conffiles": ""}
	_debMetadata2 := DebianMetadata{"Conffiles": ""}

	tests := []ParseDebianFilesResult{
		{_debMetadata1, debConffiles1, DebianMetadata{
		...
		{_debMetadata2, debConffiles2, DebianMetadata{
			"Conffiles": []map[string]interface{}{
				{
					"digest": map[string]string{
						"algorithm": "md5",
