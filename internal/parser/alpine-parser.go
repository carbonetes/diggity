...
// AlpineManifest alpine manifest
...
func parseInstalledPackages(filename string, layer string) error {
	...
	for _, apkPackage := range apkPackages {
		...
		if len(_package.Metadata.(AlpineManifest)) > 0 {
			if !*Arguments.DisableFileListing {
				_package.Metadata.(AlpineManifest)["Files"] = files
			}

			parseAlpineURL(_package)

			if _package.Metadata.(AlpineManifest)["PackageOrigin"] != nil &&
				_package.Metadata.(AlpineManifest)["PackageName"] != nil &&
				_package.Metadata.(AlpineManifest)["PackageVersion"] != nil {
				cpe.NewCPE23(_package,
					_package.Metadata.(AlpineManifest)["PackageName"].(string),
					_package.Metadata.(AlpineManifest)["PackageName"].(string),
					_package.Metadata.(AlpineManifest)["PackageVersion"].(string))
				...
				for _, content := range file.Contents {
					if strings.Contains(content.Path, _package.Metadata.(AlpineManifest)["PackageName"].(string)) {
						locations = append(locations, model.Location{
							LayerHash: content.LayerHash,
							Path:      TrimUntilLayer(*content),
						})
					}
				}
...
func parseAlpineURL(_package *model.Package) {
	arch, ok := _package.Metadata.(AlpineManifest)["Architecture"]
	if !ok {
		arch = ""
	}
	origin, ok := _package.Metadata.(AlpineManifest)["PackageOrigin"]
	if !ok {
		origin = ""
	}
