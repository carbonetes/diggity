...
func Originator(p *model.Package) string {
	...
	case parser.PythonMetadata:
		if val, ok := m["Author"]; ok {
			originator = val.(string)
		}
	case parser.DebianMetadata:
		if val, ok := m["Maintainer"]; ok {
			originator = val.(string)
		}
	case parser.GemMetadata:
...
func FormatName(image *string) string {
	...
	if image == nil {
		if *parser.Arguments.Tar != "" {
			tarFile := filepath.Base(*parser.Arguments.Tar)
			return strings.Split(tarFile, ".")[0]
		}
		if *parser.Arguments.Dir != "" {
			return filepath.Base(*parser.Arguments.Dir)
		}
	}
