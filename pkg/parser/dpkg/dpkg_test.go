...
import (
	...
	"github.com/carbonetes/diggity/pkg/parser/dpkg"
)

var (
	args   = model.NewArguments()
	target = filepath.Join("..", "..", "..", "docs", "references", "debian")
)

func TestDart(t *testing.T) {
	if _, err := os.Stat(target); os.IsNotExist(err) {
		t.Error(errors.New("Debian reference not found"))
		t.FailNow()
	}
	args.Dir = &target
	...
	req.WG.Add(1)
	dpkg.FindDpkgPackagesFromContent(req)
	req.WG.Wait()
	if len(*req.Errors) > 0 {
