package common

import (
	"errors"
	"log"
	"strings"
	"sync"

	"github.com/carbonetes/diggity/internal/ui"
	"github.com/carbonetes/diggity/pkg/docker"
	"github.com/carbonetes/diggity/pkg/files"
	"github.com/carbonetes/diggity/pkg/model"
)

type ParserParams struct {
	Arguments  *model.Arguments
	Dir        *string
	Contents   *[]model.Location
	SBOM       *model.SBOM
	DockerTemp *string
	WG         sync.WaitGroup
	Errors     *[]error
}

// InitParsers initialize arguments
func NewParams(args *model.Arguments) (*ParserParams, error) {
	if len(*args.Image) > 0 {
		if !strings.Contains(*args.Image, ":") {
			modifiedTag := *args.Image + ":latest"
			args.Image = &modifiedTag
		}
		dockerTemp := docker.CreateTempDir()
		credential := model.NewRegistryAuth(args)
		imageId := docker.GetImageID(args.Image, credential, !*args.DisablePullTimeout)
		ui.OnExtractingImage(*args.Image)
		contents, dir := docker.ExtractImage(imageId, dockerTemp)
		ui.OnScanningImage(*args.Image)
		return &ParserParams{
			Arguments:  args,
			Dir:        dir,
			DockerTemp: dockerTemp,
			Contents:   contents,
			Errors:     new([]error),
			SBOM: &model.SBOM{
				Packages: new([]model.Package),
				Secret:   new(model.SecretResults),
				Distro:   new(model.Distro),
				SLSA:     new(model.SLSA),
			},
		}, nil
	} else if len(*args.Dir) > 0 {
		if files.Exists(*args.Dir) {
			dockerTemp := docker.CreateTempDir()
			contents, err := files.GetFilesFromDir(*args.Dir)
			if err != nil {
				log.Fatal(err)
			}
			ui.OnScanningDir(*args.Dir)
			return &ParserParams{
				Arguments:  args,
				Dir:        args.Dir,
				DockerTemp: dockerTemp,
				Contents:   contents,
				Errors:     new([]error),
				SBOM: &model.SBOM{
					Packages: new([]model.Package),
					Secret:   new(model.SecretResults),
					Distro:   new(model.Distro),
					SLSA:     new(model.SLSA),
				},
			}, nil
		} else {
			log.Fatal(errors.New("Directory not found!"))
		}
	} else if len(*args.Tar) > 0 {
		if files.Exists(*args.Tar) {
			dockerTemp := docker.CreateTempDir()
			contents, dir := docker.ExtractTarFile(args.Tar, dockerTemp)
			ui.OnScanningTar(*args.Tar)
			return &ParserParams{
				Arguments:  args,
				Dir:        dir,
				DockerTemp: dockerTemp,
				Contents:   contents,
				Errors:     new([]error),
				SBOM: &model.SBOM{
					Packages: new([]model.Package),
					Secret:   new(model.SecretResults),
					Distro:   new(model.Distro),
					SLSA:     new(model.SLSA),
				},
			}, nil
		} else {
			return nil, errors.New("Tar file not found!")
		}
	} else {
		return nil, errors.New("No valid scanning target provided!")
	}
	return nil, nil
}
