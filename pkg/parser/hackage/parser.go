...
func readStackContent(location *model.Location, req *bom.ParserRequirements) {
...
func readStackLockContent(location *model.Location, req *bom.ParserRequirements) {
	stackBytes, err := os.ReadFile(location.Path)
	if err != nil {
		err = errors.New(parserErr + err.Error())
		*req.Errors = append(*req.Errors, err)
...
func readCabalFreezeContent(location *model.Location, req *bom.ParserRequirements) {
